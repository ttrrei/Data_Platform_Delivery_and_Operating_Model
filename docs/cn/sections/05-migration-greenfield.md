...
| **Migration（迁移）** | 已有 legacy 平台，需替换 | 切换风险、数据不一致、双账单 | [Domain-Driven + Dual Running](#migration-strategy) |
...
> **选域原则**：第一个域用 [§7 Value Heat Map](07-day2-operations.md#value-heat-map)（该框架定义见 §7.1.1，此处为前向引用）思路选——**高业务价值 × 高跨团队使用**的域优先，尽早证明价值。但避开「最复杂、最多合规纠纷」的域作为首发，那是第二批。
...
| **比较语义先行** | 对账开始前固定比较口径：舍入规则、时区、晚到数据截点、NULL/去重语义。否则追到的往往是口径不一致产生的**伪方差**，零方差永远不可达 |
...
| 双平台并行成常态 | Dual Running 是**有限期**手段，不是终态 |
