...
### 1.4.2 第二轴：Machine-Enforced vs. Human-Enforced（关键创新）
...
> **核心主张**：**能 machine-enforce 的，绝不留给 human-enforce。** 人工强制的约束 = 迟早被违反的约束。本轴线的产物，是给 [Section 3 治理即代码](03-modeling-governance.md#governance-as-code) 的一份「哪些约束必须自动化」的清单。
...
| `compliance_regime` | `none` / `pii-only` / `sector-regulated`（如 APRA/HIPAA/GDPR） | 合规驱动 | 约束选型的硬门槛（一票否决项） |
